...
function ensureSchema(sql) {
  if (!schemaPromise) {
    schemaPromise = (async () => {
      ...
      await sql`ALTER TABLE candidates ADD COLUMN IF NOT EXISTS gaps TEXT`;
      await sql`
      ...
      await sql`ALTER TABLE job_descriptions ADD COLUMN IF NOT EXISTS keyword TEXT`;
    })().catch((e) => {
...
export async function getCandidates(jobId) {
  ...
  if (!jobId) return [];
  return sql`
    SELECT id, name, role, stage, notes, email, resume_url, fit_score, fit_reason, gaps,
...
export async function addCandidateRow({
  ...
  await sql`
    INSERT INTO candidates
      (name, role, stage, notes, email, resume_url, fit_score, fit_reason, gaps, phone, location, rate, citizenship, job_id)
    VALUES
      (${name}, ${role || null}, ${stage}, ${notes || null}, ${email || null}, ${resumeUrl || null},
       ${fitScore}, ${fitReason || null}, ${gaps || null}, ${phone || null}, ${location || null}, ${rate || null},
...
// Bulk delete candidates for a job whose fit score is below the threshold.
export async function deleteCandidatesBelowScore(jobId, threshold) {
  const sql = getSql();
  await sql`
    DELETE FROM candidates
    WHERE job_id = ${jobId} AND fit_score IS NOT NULL AND fit_score < ${threshold}
  `;
}

// Delete every candidate for a job (clear the list to start over).
export async function deleteAllCandidatesForJob(jobId) {
  const sql = getSql();
...
export async function updateCandidateStage(id, stage) {
  const sql = getSql();
  await sql`UPDATE candidates SET stage = ${stage} WHERE id = ${id}`;
}
...
export async function getActiveJobDescription() {
  ...
  const active = await sql`
    SELECT id, filename, file_url, content, active, keyword, created_at
    FROM job_descriptions
    WHERE active = true
    ORDER BY created_at DESC
    LIMIT 1
...
export async function setJobKeyword(id, keyword) {
  const sql = getSql();
  await ensureSchema(sql);
  await sql`UPDATE job_descriptions SET keyword = ${keyword || null} WHERE id = ${id}`;
}
...
// Collapse job descriptions that share the same file name, keeping the one
// with readable text (or the most recent) and moving candidates onto it.
// Safe to run on every load — a no-op once there are no duplicates.
export async function mergeDuplicateJobs() {
  const sql = getSql();
  await ensureSchema(sql);
  const all = await sql`
    SELECT id, filename, content, active, created_at
    FROM job_descriptions
    ORDER BY created_at DESC
  `;
  const groups = new Map();
  for (const j of all) {
    if (!j.filename) continue;
    if (!groups.has(j.filename)) groups.set(j.filename, []);
    groups.get(j.filename).push(j);
  }
  for (const group of groups.values()) {
    if (group.length < 2) continue;
    const keeper = group.find((g) => g.content && g.content.trim()) || group[0];
    const hadActive = group.some((g) => g.active);
    for (const g of group) {
      if (g.id === keeper.id) continue;
      await sql`UPDATE candidates SET job_id = ${keeper.id} WHERE job_id = ${g.id}`;
      await sql`DELETE FROM job_descriptions WHERE id = ${g.id}`;
    }
    if (hadActive) {
      await sql`UPDATE job_descriptions SET active = true WHERE id = ${keeper.id}`;
    }
  }
  // Ensure exactly one active job remains.
  const active = await sql`SELECT id FROM job_descriptions WHERE active = true ORDER BY created_at DESC`;
  if (active.length === 0) {
    const latest = await sql`SELECT id FROM job_descriptions ORDER BY created_at DESC LIMIT 1`;
    if (latest[0]) await sql`UPDATE job_descriptions SET active = true WHERE id = ${latest[0].id}`;
  } else if (active.length > 1) {
    await sql`UPDATE job_descriptions SET active = (id = ${active[0].id})`;
  }
}
